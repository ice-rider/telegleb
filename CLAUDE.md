...
- [`docs/AUDIT.md`](docs/AUDIT.md) — разбор дефектов, из-за которых контракт переписывали.
  Объясняет, почему спека устроена именно так.
- [`docs/ROADMAP.md`](docs/ROADMAP.md) — план развития по этапам, что сделано и что дальше.
- [`deploy/README.md`](deploy/README.md) — прод: образы, секреты, сертификаты.
...
- **Список диалогов забирается целиком**, страницами по 100 до потолка. Частичная выборка ломает
  папки: чат, не попавший в выборку, молча выпадает из своей папки. Именно так папка из 13 чатов
  показывала 2.
- **Форум (`chat.isForum`) не имеет плоской истории.** Сообщения живут в темах: список — через
  `messages.getForumTopics`, история темы — через `messages.getReplies` с `msg_id` темы, отправка —
  с `reply_to.top_msg_id`.
...
Все ошибки клиента пишутся в консоль браузера (`src/core/log.ts`): перехватчик axios логирует
каждый сбой API с контекстом, глобальные обработчики ловят необработанные исключения и промисы,
а загрузка дашборда печатает сводку с числом чатов в каждой папке.

Незакрытое и план — в [`docs/ROADMAP.md`](docs/ROADMAP.md).
